...
def getWordScore(word, n):
	...
	for letters in word:
		if letters in SCRABBLE_LETTER_VALUES:
			sum = sum + SCRABBLE_LETTER_VALUES[letters]
	if n == len(word):
...
def displayHand(hand):
	...
	for letter in hand.keys():
		for j in range(hand[letter]):
			 ans+=letter+" "       # print all on the same line
	return ans
	...
	while calculateHandlen(hand) > 0:
		...
		if user == '.':
			...
			if isValidWord(user,hand,wordList):
				word_score = getWordScore(user, n)
				total_score += word_score
				print(user + " earned " + str(word_score) + " points. Total: " + str(total_score))
				hand = updateHand(hand, user)
			else:
				print("your word is rejected. Enter again")
	print('Run out of letters. total: ' +str(total_score)+ ' points ')
